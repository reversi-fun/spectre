...
def run_simulation(num_sensors=71, num_iterations=1, num_rounds=1000, save_fig=False):
    ...
    for round_idx in range(num_rounds):
        ...
        for network_type, network in networks.items():
            ...
            if round_idx == 0 and iteration == 0:
                plot_network_with_paths(network, results[network_type]['paths'], clone_positions, results[network_type]['detected_clones'], tuple(np.mean(network, axis=0)), f'{network_type} Network', save_fig)
    ...
    plot_metrics(results, num_rounds, save_fig)

def plot_metrics(results, num_rounds, save_fig=False):
    ...
    for metric, color in zip(metrics, colors):
        ...
        plt.tight_layout()
        if save_fig:
            plt.savefig(f'figures/{metric}_for_Each_Topology_Over_{num_rounds}_Rounds_Clone_Percentage_1.png', dpi=300)
        plt.show()
