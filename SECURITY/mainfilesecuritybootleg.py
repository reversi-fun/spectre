...
def detect_pattern(current_node, network):
    unique_angles, unique_distances = get_unique_angles_distances(current_node, network)
    return len(unique_angles), len(unique_distances)  # Return actual values for debugging
...
def run_simulation(num_sensors=71, num_iterations=1, num_rounds=10000, save_fig=False):
    ...
    for network_type, network in networks.items():
        ...
        unique_distances_set = set()
        for node in network:
            unique_angles, unique_distances = get_unique_angles_distances(node, network)
            unique_angles_set.update(unique_angles)
            unique_distances_set.update(unique_distances)
        print(f"{network_type} Network: Unique Angles = {len(unique_angles_set)}, Unique Distances = {len(unique_distances_set)}")
