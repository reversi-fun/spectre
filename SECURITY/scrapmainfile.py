...
# Adjusting RC parameters
plt.rcParams.update({
    'figure.figsize': (6, 4),   # Size of the figure
    ...
    'figure.dpi': 300,          # DPI for high-res images
    'savefig.dpi': 600,         # DPI for saved figures
})
...
def plot_network_with_paths(network, paths, clone_positions, detected_clones, base_station_position, title, save_fig=False):
    ...
    for node in network:
        ...
        plt.plot(node[0], node[1], 'bo', markersize=4, label='Uncompromised Nodes' if 'Uncompromised Nodes' not in plt.gca().get_legend_handles_labels()[1] else "")
    ...
    for pos in clone_positions:
        plt.plot(pos[0], pos[1], 'ro', markersize=6, label='Cloned Nodes' if 'Cloned Nodes' not in plt.gca().get_legend_handles_labels()[1] else "")
    
    # Plot detected cloned nodes
    for pos in detected_clones:
        plt.plot(pos[0], pos[1], 'yo', markersize=6, label='Detected Cloned Nodes' if 'Detected Cloned Nodes' not in plt.gca().get_legend_handles_labels()[1] else "")
    ...
    if save_fig:
        plt.savefig(f'figures/{title}.png', dpi=600)  # Higher DPI for saving
    plt.show()
