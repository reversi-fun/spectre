...
def smart_random_walk(network, intruder_position, visited_nodes, is_aperiodic):
    ...
    for idx in sorted_indices:
        ...
        if tuple(nearest_node) not in visited_nodes and hop_distance <= SENSOR_RADIUS:
...
def run_simulation():
    ...
    for network_type, network, base_station in zip(network_types, networks, base_stations):
        print(f"Running simulation for {network_type} network...")
        try:
            ...
            if len(path) > 1:
                ...
                print(f"No valid path found for {network_type} network.")
        except Exception as e:
            print(f"Error in {network_type} network simulation: {e}")
