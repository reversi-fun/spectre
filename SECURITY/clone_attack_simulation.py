...
def simulate_clone_attack(network, clone_positions, base_station_position):
    ...
    return detections, paths, time_steps, total_hops, detected_clones
...
def run_simulation(num_sensors=559, num_iterations=1, num_rounds=10):
    ...
    results = {network_type: {'detections': 0, 'paths': [], 'time_steps': 0, 'total_hops': 0, 'base_station_reached': 0, 'detected_clones': set()} for network_type in networks.keys()}
    ...
    for round_idx in range(num_rounds):
        ...
        for network_type, network in networks.items():
            ...
            for iteration in range(num_iterations):
                detections, paths, time_steps, total_hops, detected_clones = simulate_clone_attack(network, clone_positions, tuple(np.mean(network, axis=0)))
    ...
    for network_type in results.keys():
        print(f"{network_type} Network: {results[network_type]['detections']} detections, {results[network_type]['base_station_reached']} base stations reached out of {num_iterations * len(clone_positions_per_network[network_type]) * num_rounds} total clones")
