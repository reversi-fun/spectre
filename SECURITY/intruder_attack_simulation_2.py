...
def simulate_intruder_attack(network, intruder_position, base_station_position, network_type):
    ...
    while not has_reached_base_station(intruder_position, base_station_position) and time_steps < MAX_STEPS:
        ...
        print(f"Step {time_steps}: Intruder at {intruder_position}")
...
def smart_random_walk(network, intruder_position, visited_nodes, is_aperiodic):
    ...
    # Try to move to an unvisited node within the sensing range first
    for idx in sorted_indices:
        nearest_node = network[idx]
        if tuple(nearest_node) not in visited_nodes and np.linalg.norm(np.array(nearest_node) - np.array(intruder_position)) <= SENSOR_RADIUS:
    ...
    # If no new nodes are available, return to a previously visited node within the sensing range
    for idx in sorted_indices:
        nearest_node = network[idx]
        if np.linalg.norm(np.array(nearest_node) - np.array(intruder_position)) <= SENSOR_RADIUS:
            step_time = calculate_time_step(nearest_node, intruder_position, network)
            pattern_found = detect_pattern(nearest_node, network)
            return nearest_node, step_time, pattern_found
...
def plot_network_with_path(network, path, base_station_position, title):
    ...
    for node in network:
        ...
        comm_circle = plt.Circle(node, SENSOR_RADIUS * 2, color='orange', alpha=0.1)
