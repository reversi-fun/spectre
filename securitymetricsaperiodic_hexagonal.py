...
def generate_aperiodic_network(field_size, sensor_radius):
...
def generate_hexagonal_network(field_size, sensor_radius):
    ...
    for x in range(int(field_size / (1.5 * sensor_radius))):
        for y in range(int(field_size / hex_height)):
            center_x = x * 1.5 * sensor_radius
            center_y = y * hex_height + (x % 2) * hex_height / 2
            if center_x < field_size and center_y < field_size:
...
# Add any other necessary functions from the previous code here
